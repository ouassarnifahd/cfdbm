...
// fprintf(logfile, "# Build Version %lu\n", (unsigned long)BUILD_VERSION);
  ...
  #define ctime_date(str_TIME, str) do { \
  ...
  #define ctime_time(str_TIME, str) do { \
  ...
  #define log_printf(MSG, ...) do {\
    ...
    fprintf(stdout, "%s", log_str); log(log_str); if (__TRD__ == 1) printf_unlock(); } while(0)
#endif // __DEBUG__
